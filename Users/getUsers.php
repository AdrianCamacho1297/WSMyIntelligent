...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $result = mysqli_query($Cn, "SELECT * from users WHERE num_user = $numUser");
    ...
    if (!empty($result)) {
        if (mysqli_num_rows($result) > 0) {
            ...
            $user["name_user"] = $result["name_user"];
            $user["esp_user"] = $result["esp_user"];
            $user["sem_user"] = $result["sem_user"];
            $user["age_user"] = $result["age_user"];
            $user["sex_user"] = $result["sex_user"];
